...
const ChatMessage = ({ message }: ChatMessageProps) => {
  return (
    <div className={`flex gap-2 md:gap-3 ${message.type === 'user' ? 'justify-end' : 'justify-start'}`}>
      {message.type === 'ai' && (
        <Avatar className="w-6 h-6 md:w-8 md:h-8 border-2 border-background shadow-lg flex-shrink-0">
      ...
      <div className={`max-w-[90%] md:max-w-[75%] ${message.type === 'user'
        ...
        <div className={`text-xs mt-1 md:mt-2 ${message.type === 'user' ? 'text-blue-100' : 'text-muted-foreground'
          }`}>
