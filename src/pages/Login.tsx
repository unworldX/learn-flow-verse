...
const Login = () => {
  ...
  // Helper to safely extract redirect path from router state
  function getFromPath(loc: Location): string | null {
    const state = loc.state as { from?: { pathname?: string } } | null;
    return state?.from?.pathname ?? null;
  }

  // If already logged in and visiting /login, bounce to home
  useEffect(() => {
    if (user) {
